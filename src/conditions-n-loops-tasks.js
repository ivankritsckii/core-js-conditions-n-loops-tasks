...
function isContainNumber(num, digit) {
  for (let i = num; i > 1; Math.floor((i /= 10))) {
    if (Math.floor(i % 10) === digit) {
      return true;
    }
  }
  return false;
...
function getSpiralMatrix(size) {
  ...
  for (let zn = 1; zn <= size * size; zn += 1) {
    if (width === iterI) {
      arr[width][height] = zn;
      height += 1;
      if (height === size - iterJ) {
        width += 1;
        height -= 1;
      }
    } else if (height === size - iterJ - 1) {
      arr[width][height] = zn;
      width += 1;
      if (width === size - iterI) {
        height -= 1;
